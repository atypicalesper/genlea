...
export const normalizer = {
  normalizeCompany(raw: Partial<import('../types/index.js').RawCompany>, source: ScraperSource): Partial<Company> {
    ...
    return {
      ...
      hqCountry: raw.hqCountry,
      hqState: raw.hqState?.trim(),
      hqCity: raw.hqCity?.trim(),
      employeeCount: raw.employeeCount ? parseInt(String(raw.employeeCount), 10) || undefined : undefined,
...
function normalizeUrl(url?: string): string | undefined {
  ...
  if (!trimmed || trimmed.startsWith('mailto:') || trimmed.startsWith('javascript:')) return undefined;

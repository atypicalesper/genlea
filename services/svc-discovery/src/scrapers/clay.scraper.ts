...
import type { Scraper, ScrapeQuery, RawResult, RawCompany, RawContact } from '@genlea/shared';
  ...
  async enrichDomain(domain: string, companyName?: string): Promise<RawResult | null> {
    ...
    try {
      ...
      const company: Partial<RawCompany> = {
        ...
        fundingStage:  normalizeFunding(cd.funding_stage ?? cd.last_funding_type) as import('@genlea/shared').FundingStage | undefined,

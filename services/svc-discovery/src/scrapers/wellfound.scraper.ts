...
import { generateRunId, randomBetween } from '@genlea/shared';
...
export class WellfoundScraper implements Scraper {
  ...
  private async scrapeFounders(
    page: Page,
    domain: string
  ): Promise<Partial<import('@genlea/shared').RawContact>[]> {
    const founderCards = await page.$$('[data-test="founder"], [class*="founder"], [class*="team-member"]');
    const contacts: Partial<import('@genlea/shared').RawContact>[] = [];
...
function mapStage(stage?: string): import('@genlea/shared').FundingStage {

...
import type { LeadStatus, LeadFilter } from '@genlea/shared';
...
function buildLeadsFilter(q: LeadQuery): Record<string, unknown> {
  ...
  if (q.minScore || q.maxScore) {
    ...
    const minN = parseInt(String(q.minScore ?? ''), 10);
    const maxN = parseInt(String(q.maxScore ?? ''), 10);
    if (!isNaN(minN)) scoreFilter['$gte'] = minN;
    if (!isNaN(maxN)) scoreFilter['$lte'] = maxN;
    if (Object.keys(scoreFilter).length > 0) filter['score'] = scoreFilter;
  }

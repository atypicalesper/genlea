...
export function resolveStatus(
  ...
  const hot         = thresholds?.hot         ?? parseInt(process.env['SCORE_HOT']          ?? '65', 10);
  const warm        = thresholds?.warm        ?? parseInt(process.env['SCORE_WARM']         ?? '50', 10);
  const cold        = thresholds?.cold        ?? parseInt(process.env['SCORE_COLD']         ?? '35', 10);
